  ...
  for (let i = 0; i < productsData.length; ++i) {
    ...
    productsContainer.innerHTML += `
      ...
      return htmlData;
                ...
                <p class="product-cost">$ ${getMinPrice(productsData[i].variation).toFixed(1)}</p>
            </div>
        `;
  }
}
...
function openProductDetails(id) {
  ...
  for (let i = 0; i < productsData[id].variation.length; ++i)
    articles += `<option>${prodId}-${String.fromCharCode(65 + i)} - ${productsData[id].variation[i].title[lang]}: $${productsData[id].variation[i].price.toFixed(1)}</option>\n`;
...
function updateCart() {
  ...
  for (let i = 0; i < cart.length; ++i) {
    item = getCartItemDetails(i);
    totalProductsCost += item.qnt * item.price;
...
function updateShippingCost() {
  ...
  totalShippingCost = totalShippingCost == 0 ? 0 : Math.ceil(totalShippingCost + totalProductsCost * insurance);
}
...
function updateCartTotals() {
  ...
  document.getElementById('checkout-grand-total').innerHTML = `${i18n[lang]['inner-checkout-grand-total']}: $${totalProductsCost + totalShippingCost}`;
